...
def kendra_search(query):
    ...
    high_conf_li = []
    low_conf_li = []
    index = "enter_your_aws_index_here"
...
class Prompt:
    ...
    def set_pre_prompt(self, preprompt):
        self.system = preprompt
...
def kendra_to_gpt3_search(intent_request, sess_id):
    ...
    confirm_stage = managesess.get_sess_confirmstage(sess_id)
    cx_question_hist = managesess.get_sess_cx_hist(sess_id)
    bot_reply_hist = managesess.get_sess_bot_hist(sess_id)
    ...
    if cx_question == "CLEAR" or cx_question == "RESET":
        ...
        need_agent, confirm_stage, cx_question_hist, bot_reply_hist = init_revchatgpt()

        if cx_question == "RESET":
            managesess = SessConvID()
    else:
        cx_question_hist.append(cx_question)

        if not confirm_stage:
            if not need_agent:
                highconf_list, lowconf_list, need_agent = dbquery_chain(cx_question, need_agent)

            if not need_agent:
                first_stage_rep, need_agent = extractans_chain(cx_question, highconf_list, lowconf_list, need_agent, sess_id)

            if not need_agent:
                need_agent, confirm_stage, cx_question_hist, bot_reply_hist, reply_to_user = nlg_chain(first_stage_rep, need_agent, confirm_stage, cx_question_hist, bot_reply_hist, sess_id=sess_id)
            else:
                need_agent, confirm_stage, cx_question_hist, bot_reply_hist, reply_to_user = transfer_chain(need_agent, confirm_stage, cx_question_hist, bot_reply_hist, sess_id=sess_id)
        else:
            confirm_transfer = chk_user_want_agent(cx_question_hist, bot_reply_hist, sess_id)
            confirm_stage = False
            need_agent = False
            ...
            if 'No' in confirm_transfer:
                ...
                if not need_agent:
                    first_stage_rep, need_agent = extractans_chain(cx_question, highconf_list, lowconf_list, need_agent, sess_id=sess_id)

                if not need_agent:
                    need_agent, confirm_stage, cx_question_hist, bot_reply_hist, reply_to_user = nlg_chain(first_stage_rep, need_agent, confirm_stage, cx_question_hist, bot_reply_hist, sess_id)
                else:
                    need_agent, confirm_stage, cx_question_hist, bot_reply_hist, reply_to_user = transfer_chain(need_agent, confirm_stage, cx_question_hist, bot_reply_hist, sess_id)

    fulfillment_state = "Fulfilled"

    if "OpenAI/API outage" in reply_to_user:
        reply_to_user, need_agent, confirm_stage, cx_question_hist, bot_reply_hist = kendra_to_gbt3_search(intent_request, sess_id, reply_to_user, need_agent, confirm_stage, cx_question_hist, bot_reply_hist)
        managesess.set_sess_needagent(sess_id, need_agent)
        managesess.set_sess_confirmstage(sess_id, confirm_stage)
        managesess.add_sess_cx_q(sess_id, cx_question_hist.copy())
        managesess.add_sess_bot_a(sess_id, bot_reply_hist.copy())
    else:
        managesess.set_sess_needagent(sess_id, need_agent)
        managesess.set_sess_confirmstage(sess_id, confirm_stage)
        managesess.add_sess_cx_q(sess_id, cx_question_hist.copy())
